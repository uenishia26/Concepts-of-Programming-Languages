...
let next_token (cs : char list) : (token * char list) option =
  let cs = snd (span is_blank cs) in (* Step 1: Remove leading whitespace *)
  match cs with
  | [] -> Some (EOFT, []) (* End of input *)
  | ':' :: ':' :: '=' :: rest -> Some (EqT, rest) (* ::= *)
  | '.' :: rest -> Some (PdT, rest) (* . *)
  | '<' :: _ -> (* Nonterminal symbol *)
    (match span (fun c -> c <> '>') cs with
     | (_, []) -> None (* Invalid token: unclosed < *)
     | ('<' :: sym, '>' :: rest) when List.for_all is_lower_case sym ->
       Some (NtmT (implode sym), rest)
     | _ -> None)
  | c :: _ when is_lower_case c -> (* Terminal symbol *)
    let sym, rest = span is_lower_case cs in
    Some (TmT (implode sym), rest)
...
let rec expand_leftmost ((nt, sf) : rule) (s : sentform) : sentform =
  match s with
  | [] -> [] (* Base case: if the list is empty, return empty *)
  | (NT x as head) :: tail ->
    if x = nt then
      sf @ tail (* If the head is the matching NT, replace it with `sf` *)
    else
      head :: expand_leftmost (nt, sf) tail (* Continue searching in the tail *)
  | head :: tail ->
    head :: expand_leftmost (nt, sf) tail (* Continue searching if head is not NT *)
...
let rec parse_sentform (ts : token list) : (sentform * token list) option =
  ...
  | NtmT id :: rest ->
    (match parse_sentform rest with
     | None -> Some ([NT id], rest) 
     | Some (sf, remaining) -> Some (NT id :: sf, remaining))
  | TmT id :: rest ->
    (match parse_sentform rest with
     | None -> Some ([T id], rest) 
     | Some (sf, remaining) -> Some (T id :: sf, remaining))
  | _ -> None  

let parse_rule (ts : token list) : (rule * token list) option =
  match ts with
  | NtmT id :: EqT :: PdT :: remaining -> Some ((id, []), remaining) 
  | NtmT id :: EqT :: rest ->
    (match parse_sentform rest with
     | Some (sf, PdT :: remaining) -> Some ((id, sf), remaining)
     | _ -> None)
  | _ -> None
...
let rec parse_grammar (ts : token list) : grammar * token list =
  ...
  | Some (rule, remaining) ->
    let (g, final_remaining) = parse_grammar remaining in
    (rule :: g, final_remaining)  
  | None -> ([], ts) 
...
let parse_and_check (s : string) : grammar option =
  ...
  | Some ts ->
    ...
    if List.length rest = 0 (*List.isempty same as List.length rest = 0/ Kept getting unbound error*)
    then Some g
    else None
...
let () =
  match parse_rule [NtmT "ntOnly"; EqT; NtmT "a"; NtmT "b"; PdT] with
  | Some (("ntOnly", [NT "a"; NT "b"]), []) -> ()
  | _ -> assert false
